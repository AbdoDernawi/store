...
export async function GET(
  ...
  const { data: chat, error } = await auth.context.supabase.rpc("get_or_create_marketer_chat", {
  ...
  const chatId = (chat as { id?: string; is_open?: boolean } | null)?.id;
...
export async function POST(
  ...
  const chatInfo = chat as { id?: string; is_open?: boolean } | null;

  if (!chatInfo?.id || !chatInfo.is_open) {
