...
export function MarketerOrderActions({
  cities,
  order,
  zones,
}: {
  cities: MarketerCity[];
  order: MarketerOrderDetails["order"];
  zones: MarketerZone[];
}) {
  ...
  async function submitEdit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/marketer/orders/${order.id}`, {
      method: "PATCH",
  ...
  async function submitCancel(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/marketer/orders/${order.id}/cancel-request`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-4">
      {canEdit ? (
        ...
        >
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-base font-black text-slate-950">تعديل الطلب</h3>
          </div>

          <div className="space-y-3">
            <input className={inputClass} defaultValue={order.customer_name} name="customer_name" placeholder="اسم الزبون" />
            <input className={inputClass} defaultValue={order.customer_phone} name="customer_phone" placeholder="رقم الهاتف" />
      ...
      {canRequestCancel ? (
        ...
        <div className="rounded-lg bg-amber-50 p-4 text-sm font-black text-amber-700 ring-1 ring-amber-100">
