...
export function MarketerWalletGate() {
  const router = useRouter();
  const [data, setData] = useState<MarketerWalletData | null>(null);
  ...
  async function unlockWallet(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/marketer/wallet/unlock", {
      body: JSON.stringify({ code: walletCode }),
      headers: { "Content-Type": "application/json" },
      method: "POST",
    });
    const result = (await response.json().catch(() => ({}))) as { error?: string; wallet?: MarketerWalletData };

    if (!response.ok || result.error || !result.wallet) {
      setUnlockError(result.error || "رمز المحفظة غير صحيح.");
      return;
    }

    setWalletCode("");
    setData(result.wallet);
    setRequests(result.wallet.withdrawalRequests);
  }
      ...
      <section className="mx-auto max-w-xl rounded-[1.7rem] border border-emerald-100 bg-white p-5 shadow-sm shadow-slate-950/5">
        <div className="mb-5 flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black text-slate-950">أدخل رمز المحفظة لعرض الرصيد</h2>
            <p className="mt-2 text-sm font-bold leading-6 text-slate-500">
              الرصيد وسجل التحويلات لا يظهران إلا برمز المحفظة الذي يعيّنه الأدمن.
            </p>
          </div>
        </div>
